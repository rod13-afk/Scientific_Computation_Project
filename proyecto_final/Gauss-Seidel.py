...
def encontrar_soluciones(ecuaciones, precision):
    def iteracion(aproximadores, soluciones):
        ...
        for aproximacion in aproximadores:
            soluciones_iteracion[aproximacion[0]] = ((aproximacion[1]) - (
                sum(array(soluciones) * array(aproximacion[3])))) / (aproximacion[2])
...
n = int(input("¿Cuantas variables tienes?\n"))
precision = int(input("¿Para cuántos decimales de precisión quieres que se calculen tus soluciones?\n"))
